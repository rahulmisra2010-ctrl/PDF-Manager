...
class ExportService:
    ...
    def _export_csv(self, doc, fields, path):
        with open(path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['Document', 'Field Name', 'Field Value', 'Confidence', 'Page', 'Edited'])
            for field in fields:
                writer.writerow([
                    doc.filename,
                    field.field_name,
    ...
    def _export_xlsx(self, doc, fields, path):
        ...
        headers = ['Document', 'Field Name', 'Field Value', 'Confidence', 'Page', 'Edited']
        ...
        for row, field in enumerate(fields, 2):
            ...
            ws.cell(row=row, column=3, value=field.value)
            ws.cell(row=row, column=4, value=round(field.confidence, 3))
            ws.cell(row=row, column=5, value=getattr(field, 'page_number', None))
