...
GET    /training/list             — list examples (filterable)
DELETE /training/<id>             — remove a single training example
"""
...
def add_training():
    ...
    Accepts JSON body with either a dict or list for ``fields``::

        # Dict form (field_name → value)
        {"document_id": 1, "fields": {"Name": "Rahul Misra", ...}}

        # List form
        {"document_id": 1, "fields": [{"field_name": "Name", "field_value": "Rahul Misra"}, ...]}
    ...
    # Normalise fields to a list of {field_name, field_value} dicts
    ...
    added = 0
    saved_items: list = []
    # Replace existing examples for this document before adding new ones
    ...
    for item in fields:
        ...
        added += 1
        saved_items.append({"field_name": field_name, "field_value": field_value})
    ...
    return jsonify({"ok": True, "added": added, "document_id": document_id, "saved": saved_items})
...
@login_required
def list_training():
    """Return stored training examples, optionally filtered.
